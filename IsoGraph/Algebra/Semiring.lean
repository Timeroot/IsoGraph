...
/-!
# The semiring of isomorphism classes

Isomorphism classes of finite graphs form a commutative semiring under disjoint union and any one
of the four products.  This file bundles the structures, fixes the scopes that select a product, and
records that both sums are cancellative.
-/
...
`Core/Quotient.lean`.  They are still definitions, not instances — the scopes come next. -/
...
namespace IsoGraph.TensorProduct
...
namespace IsoGraph.TensorSemiring
...
namespace IsoGraph.LexSemiring
...
end IsoGraph.LexSemiring

namespace IsoGraph.JoinLexSemiring
