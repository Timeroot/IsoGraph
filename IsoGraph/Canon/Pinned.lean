    ...
    · rename_i hj
      ...
      refine splitCellsFrom_pinned hnd fuel (j + 1) _ (splitCell_inv hinv h1 h2)
        (fun j' ha hb => ⟨(hst j' (by omega) hb).1, splitCell_start hinv h1 h2
          (hst j' (by omega) hb).1 (nodup_getElem!_ne hnd hb hjs (by omega))
          (hst j' (by omega) hb).2⟩)
...
same node. -/
theorem Node.det {n : Nat} {f : Nat → Nat → Bool} {path : Array Nat} {invPath invPath' : Array UInt64}
