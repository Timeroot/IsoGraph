...
group — the search prunes, and `St.addAuto` stops recording after `maxGens` of them — and that is
why the transitivity tests return a `Cert P` ("a proof of `P`, or nothing") rather than a
`Decidable P`.  A `Cert` is data, so it can be produced by the compiled search and consumed by
`native_decide`:
...
If a *complete* generating set is what you need, `Canon/Chain.lean` computes one and proves it
complete: `autGroup n adj = Subgroup.closure (permsOf n (chainArrays n adj))`, equivalently
`Subgroup.closure {x | x ∈ fullGens n adj} = ⊤`.  It pays for that with a full subtree search per
candidate point, so it is a reference implementation and a proof, not the fast path.
-/
