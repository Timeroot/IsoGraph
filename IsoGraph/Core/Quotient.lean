...
/-- The join respects isomorphism: the same bijection as for the disjoint union, the edges across
being there on both sides whatever it does. -/
@[toIsoGraph]
def join
  ...
  isoOfAdj (G := G ∇g H) (H := G' ∇g H')
    (Equiv.sumCongr i.toEquiv j.toEquiv) fun x y ↦ by
      rcases x with a | b <;> rcases y with c | d
      · show (G' ∇g H').Adj (.inl (i a)) (.inl (i c)) = _
        rw [join_adj_inl_inl, i.adj_eq]; rfl
      · show (G' ∇g H').Adj (.inl (i a)) (.inr (j d)) = _
        rfl
      · show (G' ∇g H').Adj (.inr (j b)) (.inl (i c)) = _
        rfl
      · show (G' ∇g H').Adj (.inr (j b)) (.inr (j d)) = _
        rw [join_adj_inr_inr, j.adj_eq]; rfl
...
and to the right, so `G ⊕g H □g K ^g L` is `G ⊕g (H □g (K ^g L))`.  Every operation on classes is
generated by one of the `@[toIsoGraph]` attributes above, so all eight tokens can be declared
together here. -/
...
def lineGraph (i : G ≃cg G') :
  ...
  isoOfAdj (G := CGraph.lineGraph G) (H := CGraph.lineGraph G')
    i.toSimpleIso.mapEdgeSet fun e f ↦ by
      have hmem : ∀ (x : (CGraph.lineGraph G).V) (v : G'.V),
          v ∈ ((i.toSimpleIso.mapEdgeSet x).1 : Sym2 G'.V) ↔ ∃ u, u ∈ (x.1 : Sym2 G.V) ∧ i u = v := by
        intro x v
        show v ∈ Sym2.map (fun v ↦ i v) x.1 ↔ _
        rw [Sym2.mem_map]
      ...
      · refine decide_eq_decide.2 ⟨?_, ?_⟩
        · rintro ⟨v, hv1, hv2⟩
          ...
          rwa [hba] at hb
        · rintro ⟨v, hv1, hv2⟩
          exact ⟨i v, (hmem e _).2 ⟨v, hv1, rfl⟩, (hmem f _).2 ⟨v, hv2, rfl⟩⟩
