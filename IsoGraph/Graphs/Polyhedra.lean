...
/-!
# Polyhedra, by their faces

A convex polyhedron is a combinatorial object before it is a geometric one, and the combinatorial
object is small: the list of faces, each face being the list of its vertices in cyclic order.  The
one convention is that the faces are oriented consistently, so that the two faces meeting along an
edge traverse that edge in opposite directions; then every edge of the polyhedron occurs exactly
once as a pair `(a, b)` with `a < b` among the pairs consecutive round a face, which is what
`faceEdges` collects.

From the face list the classical operations follow by counting, with no table of data anywhere:
`truncEdges` cuts every corner off, `rectEdges` cuts them off as far as the edge midpoints,
`kisEdges` raises a pyramid on every face, and `incidenceEdges` joins each vertex to the faces
through it, which is the rhombic solid halfway between the polyhedron and its dual.  The five
Platonic face lists at the end of the module are the only data, and the Archimedean and Catalan
solids of `IsoGraph/Graphs/NamedSolids.lean` are the operations above applied to them.
-/
...
and the icosahedron are in `IsoGraph/Graphs/NamedGraphs.lean`. -/
