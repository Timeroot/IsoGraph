...
The four products bind more tightly than the two sums, and the exponential more tightly still and
to the right, so `G ⊕g H □g K ^g L` is `G ⊕g (H □g (K ^g L))`.  Six of the seven operations on
classes are generated by the `@[toIsoGraph]` attributes above, so their tokens can be declared
