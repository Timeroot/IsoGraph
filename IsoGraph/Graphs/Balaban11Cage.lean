...
The unique `(3, 11)`-cage, on a hundred and twelve vertices, and the only cubic cage of odd girth
above seven, so the only one here that is not bipartite.

It has a module of its own because its girth is the single most expensive check in the library:
ruling out every cycle shorter than eleven walks `112 · 3 · 2⁹` closed walks through the
neighbour table, and that and the Tutte 12-cage are the only two declarations in the gallery that
need more than the default heartbeat budget.  In its own module the check runs in parallel with
the rest of the gallery instead of in series with it.

The graph is given by its LCF code, spelled out as `ofEdges 112 (lcfEdges balaban11CageCode 1)`
rather than as `lcf balaban11CageCode 1` so that the vertex type is literally `Fin 112`; the two
are the same graph.
-/
