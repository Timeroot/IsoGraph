...
The named graphs: the cubic cages, the graphs given by LCF codes, the generalized Petersen family,
and the sporadic ones — Petersen, Heawood, Möbius–Kantor, Pappus, Desargues, Nauru, Coxeter and the
rest.
-/
...
`GQ(2,2)`.  The `(3,5)`-cage is the Petersen graph, in `IsoGraph/SmallGraphs/Defs/SRG.lean`. -/
...
lists are checked in `IsoGraph/SmallGraphs/Defs/Polyhedra.lean`. -/
