...
/-!
# The connected graphs on at most six vertices

Every connected graph on at most six vertices, one definition each, organised by order and then by
the number of cycles.  The list is proved complete against the enumeration in `IsoGraph.Enum`.
-/
