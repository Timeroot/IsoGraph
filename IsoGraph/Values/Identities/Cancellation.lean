...
/-!
# Cancellation of the strong product

`Identities/Semiring.lean` proves that both sums cancel, and that the tensor product does not.
This module settles the strong product: if `A ⊠g B = A ⊠g C` and `A` has a vertex then `B = C`.
So `IsCancelMulZero` holds for `⊠g`, and the strong semiring — the disjoint union with the strong
product — is an `IsDomain`.

## The argument

Counting homomorphisms `X → G` will not do it.  Hom counts are multiplicative over the *tensor*
product, not over `⊠g`, and a hom count can be zero, so a common tensor factor cannot be divided
out — which is exactly the failure recorded as `not_isCancelMulZero_tensorProduct`.  What works is
to count homomorphisms of the *reflexive closures*: `IsRHom X G f` asks only that `f` send each
edge of `X` to an edge of `G` or else collapse it to a single vertex.  Three things then line up.

* `rhomCount X (G ⊠g H) = rhomCount X G * rhomCount X H`, since two vertices of `G ⊠g H` are
  equal-or-adjacent exactly when both of their coordinate pairs are: the strong product is the
  categorical product for these maps (`rhomCount_strongProduct`).
* `1 ≤ rhomCount X G` as soon as `G` has a vertex (`one_le_rhomCount`), because the constant maps
  are reflexive homomorphisms where they need not be homomorphisms.  That is what makes the common
  factor divisible.
* Equal reflexive-hom counts against *every* `X` force an isomorphism
  (`nonempty_iso_of_rhomCount_eq`).  This is Lovász's argument (*Operations with structures*, Acta
  Math. Acad. Sci. Hungar. 18 (1967) 321–328): every map out of `X` is a surjection onto a quotient
  of `X` followed by an injection, so `rhomCount X G` is the sum of `injHomCount (X/P) G` over the
  partitions `P` of the vertices of `X` (`rhomCount_eq_sum`).  Strong induction on the number of
  vertices peels off the trivial partition, whose quotient is `X` itself, and leaves
  `injHomCount X B = injHomCount X C` for every `X`.  Taking `X = B` and `X = C` produces injective
  homomorphisms in both directions, and a pair of those between finite graphs is an isomorphism
  (`nonempty_iso_of_injHoms`, by iterating `g ∘ f` to the order of the permutation it induces).

A partition of `X.V` is encoded here not as a `Setoid` but as a *block map*: an idempotent
`q : X.V → X.V` sending every vertex to one no larger than itself in the enumeration order, so
that `q x` is the least member of the block of `x`.  These are a `Finset` of a function type, so
the sum over partitions is an ordinary `Finset.card_eq_sum_card_fiberwise` fibred by
`blockMap f`, the map taking `x` to the least vertex with the same image; and the quotient graph
`quot X q` lives on the subtype `{v // q v = v}` rather than on a genuine quotient type.

## What is not here

The cartesian product cancels a nonempty factor too (Imrich, Klavžar and Rall, *Cancellation
properties of products of graphs*, Discrete Applied Mathematics 155 (2007) 2362–2364), but no
counting argument of this shape reaches it: the proof runs through unique prime factorisation of
connected graphs, Sabidussi 1960 and Vizing 1963, which is not formalised here.  Neither is
two-sided cancellation of the lexicographic product (Dörfler and Imrich, 1972).
-/
