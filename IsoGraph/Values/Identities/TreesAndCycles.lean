...
/-!
# Tadpoles, lollipops, spiders and theta graphs

The families built by decorating a cycle or a tree: the tadpole and the lollipop, the double
star, the spider, the theta graph, and the cycle with pendant paths.  They are parameterised by
lists of leg lengths, so almost every entry is a statement about a list, and the degenerate
parameters — a leg of length zero, no legs at all — are what most of the work goes into.
-/
