...
/-!
# The graph exponentials

Two of them: `G ^g H`, the plain one, and `G ^hg H`, the reflexive one.
...
diagonal is deleted, and almost none of the calculus survives.  What is left is the first half of
this module: the vertex count `(G ^g H).V = G.V ^ H.V`, the degenerate cases, counterexamples to
the laws one would expect, and one genuine theorem about a complete base — `K₂ ^g G` is a single
edge together with isolated vertices, for every connected `G`.

The second half is `G ^hg H`, which puts the loops back: its vertices are the *homomorphisms* for
`adjR`, the adjacency of a graph with a loop added at every vertex, and two of them are adjacent
when they are `adjR`-compatible on every `adjR`-edge of `H`.  Being a homomorphism is exactly being
`adjR`-adjacent to oneself, so deleting the diagonal loses nothing, and all three exponent laws
hold — with the **strong** product throughout, since the strong product is what the categorical
product of reflexive graphs becomes when the loops are stripped off again.
...
| `complete_two_exponential_adj` | the edges of `K₂ ^g G`, for `G` connected |

## What holds, for `^hg`

| identity | |
| --- | --- |
| `homExponential_homExponential` | `(A ^hg B) ^hg C = A ^hg (B ⊠g C)` |
| `homExponential_disjUnion` | `A ^hg (B ⊕g C) = A ^hg B ⊠g A ^hg C` |
| `strongProduct_homExponential` | `(A ⊠g B) ^hg C = A ^hg C ⊠g B ^hg C` |
| `homExponential_empty_one` | `G ^hg empty 1 = G` |
| `homExponential_empty_zero` | `G ^hg empty 0 = empty 1` |
| `empty_one_homExponential` | `empty 1 ^hg G = empty 1` |
| `complete_homExponential` | `complete m ^hg G = complete (m ^ G.V)` |
| `empty_zero_homExponential` | `empty 0 ^hg G = empty (0 ^ G.V)` |
| `empty_homExponential_of_isConnected` | `empty m ^hg G = empty m` for `G` connected |

Read the first three with `⊕g` for `+` and `⊠g` for `×`: they are `(a^b)^c = a^(b×c)`,
`a^(b+c) = a^b × a^c` and `(a×b)^c = a^c × b^c`, with `empty 1` for the unit of `⊠g` and `empty 0`
for the unit of `⊕g`, which is what the two degenerate laws above say.  The one thing the reflexive
exponential does *not* give is a closed form for its vertex count: the vertices are the
homomorphisms, and there is no formula for how many of those there are.  The two computable
extremes are the last three lines — everything is a homomorphism into a complete graph, and only
the locally constant maps are homomorphisms into an edgeless one, so an edgeless base gives
`empty (m ^ numComponents G)` (proved here only for connected `G`, where the sum law and
`numComponents` would recover the general statement).

## The literature

`^g` is the standard exponential graph of El-Zahar–Sauer and of Hell–Nešetřil's *Graphs and
Homomorphisms* §2.6 — in the loop-allowed category, where a loop at `f` means `f` is a proper
colouring, so that `K_k ^g G` has a loop exactly when `G` is `k`-colourable.  `^hg` is the
exponential of the category of reflexive graphs, right adjoint to the reflexive categorical
product, and the loopless shadow of that product is the strong product.  There is no third
convention worth adding: of the four products here, only the cartesian and the categorical give a
closed monoidal structure on loopless graphs (arXiv:2310.00493), and the categorical one is `^g`,
whose laws the missing diagonal breaks.  The two variants that suggest themselves first — `G ^g Hᶜ`
and `(G ^g H)ᶜ` — were checked by exhaustive search on small graphs and fail every law.
...
`^g` and `^hg` are the notations for the two operations, on `CGraph` and on `IsoGraph`, and are
always available.  The scope `IsoGraph.Exponential` re-tags `^g` as the `Pow` instance of both
types, so that `G ^ H` with a graph in the exponent means `G ^g H`; `IsoGraph.HomExponential` does
the same for `^hg`, and the two scopes are not meant to be opened together.  Both are deliberately
separate from the scopes of `Identities/Semiring.lean`: opening one of them alongside a product
leaves `G ^ (n : ℕ)` the monoid power of that product, and `G ^ (H : IsoGraph)` the exponential.
-/
