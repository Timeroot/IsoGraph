...
theorem markP_set {mark : Array Bool} {w : Nat} (hm : MarkP P mark) (hw : P w) :
  ...
  by_cases hxw : x = w
  · exact hxw ▸ hw
  · exact hm x (by rwa [getElem!_set!_ne hxw] at hx)
...
theorem stackP_push {stack : Array Nat} {w : Nat} (hs : StackP P stack) (hw : P w) :
  ...
  · rw [push_getElem!_lt stack w h1]; exact hs i h1
  · have : i = stack.size := by omega
    subst this
    rw [push_getElem!_eq]; exact hw
...
theorem orbitClosure_P {n : Nat} {gens : Array (Array Nat)} {seed : Array Nat}
  ...
  rw [orbitClosure]
  refine closureLoop_P hgen _ _ _ ?_ (fun i hi => hseed _ (getElem!_mem hi))
