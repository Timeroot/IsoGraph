...
A standalone script (not part of the library build) that writes `testing/invariant_coverage.txt`.
Run it from the repository root with

```
lake build && lake env lean testing/Coverage.lean
```
...
Generated by testing/Coverage.lean (`lake env lean testing/Coverage.lean` from the repository
root).
Do not edit by hand.
...
({d.invariants.size} invariants × {d.operators.size} operators + {d.families.size} families).\n"
  ...
  IO.FS.writeFile "testing/invariant_coverage.txt" out
  IO.println s!"wrote testing/invariant_coverage.txt: {filled}/{cells} cells"
